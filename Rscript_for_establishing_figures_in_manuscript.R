...
dev.off()
# ----------------------------------------------- 
# Figure 6C, Evaluation of statistically significant variants. (A) Training and validation performance of the Genotype model across epochs, based on 5-fold cross-validation. (B) Estimated heritability of significant SNPs (P-value < 0.05) identified by different models for the LMP trait. (C) Genomic prediction accuracy (measured by Pearson correlation coefficient, PCC) calculated using rrBLUP model for different SNP sets: I) Ten sets of 11,084 SNPs randomly selected across the genome (mean PCC reported). II) 4,850 SNPs with GWAS P-value < 1.0e-6. III) 11,083 SNPs with GWAS P-value < 1.0e-5. IV) 24,833 SNPs with GWAS P-value < 1.0e-4. V) 11,084 potential causal variants identified by DeepAnnotation (including 10,041 non-coding or cis-regulatory variants and 1,043 coding or RNA secondary structure-related variants with P-value < 0.01 from the backtracking strategy). VI) 644,964 SNPs located within 1Mb of the 11,084 DeepAnnotation predicted causal variants.
