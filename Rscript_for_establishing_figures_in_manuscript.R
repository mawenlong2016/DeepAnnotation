...
dev.off()
#--------------------------------------------------------------
# Figure 2C, Top-ranked PCC values between the predicted and observed phenotypic values of top-ranked samples from top 1% to top 10%. The paired t-test P-values were displayed on the black boxes with coral represents significance with P-value < 0.05. 
...
library(pheatmap)
pheatmap(cor_spatial_result[1:10,c(2,3,4,5,6,9,7,8,10,11,12)], cluster_rows = F, cluster_cols = F, scale='none', border_color = NA)
dev.off()
...
#--------------------------------------------------------------
# Figure 3B,The distribution of PCC scores of top 1%~10% ranked samples under the distribution of 5 CV results.
cor_spatial_result_cv <- NULL
for(cv in 1:ncol(pred_phenotype_Network)){
	...
	cor_spatial_result_cv[[cv]] <- cor_spatial_result
}
cor_spatial_result <- (cor_spatial_result_cv[[1]]+cor_spatial_result_cv[[2]]+cor_spatial_result_cv[[3]]+cor_spatial_result_cv[[4]]+cor_spatial_result_cv[[5]])/5
t(cor_spatial_result[c(1,5,10,20,30,40,50,60),])
cor_spatial_pvalue <- NULL
for(k in 2:8){
	cor_spatial_pvalue <- c(cor_spatial_pvalue, t.test(cor_spatial_result[1:10,9], cor_spatial_result[1:10,k], alternative = 'greater', paired=T)$p.value)
}
cor_spatial_pvalue
pdf("./figure_plot/LMP_Figure3B_spartial_PCC.pdf",height=16, width=12)
vivo_list <- list()
for(i in 2:ncol(cor_spatial_result)){
	vivo_list[[i]] <- as.numeric(cor_spatial_result[1:10,i])
}
names(vivo_list) <- c("rrBLUP", "LightGBM", "KAML", "BLUP", "BayesR", "MBLUP", "BayesRC","Network")
value_result_mean <- apply(cor_spatial_result[1:10,2:9],2,mean)
library(vioplot)
library(RColorBrewer)
col_break = brewer.pal(12, "Set3")
...
dev.off()
#--------------------------------------------------------------
# Figure 3C, The averaged PCC scores of different top-ranked samples under 5 CV results.
rank_flag <- cbind(c(0,1),c(0,10),c(10,60),c(60,100))
set.seed(0)
rank_mae <- rank(pred_phenotype_cal[,1], ties.method="max") 
rank_mae <- length(rank_mae) - rank_mae + 1
ind_sparial_pcc <- list()
for(cv in 1:ncol(pred_phenotype_Network)){
	pred_phenotype_cal <- cbind(pred_phenotype_test[,1],pred_phenotype_test[,cv+1],pred_phenotype_test_lightgbm[,cv+1],pred_phenotype_test_KAML[,cv+1],pred_phenotype_test_BLUP[,cv+1],pred_phenotype_test_BayesR[,cv+1],pred_phenotype_test_MBLUP[,cv+1],pred_phenotype_test_BayesRC[,cv+1], pred_phenotype_Network[,cv])
	colnames(pred_phenotype_cal) <- c("real", "rrBLUP", "LightGBM", "KAML", "BLUP", "BayesR", "MBLUP", "BayesRC","Network")
	ind_sparial_pcc_cv <- NULL
	for(k in 1:ncol(rank_flag)){
		pcc_temp <- NULL
		sample_flag <- which( (rank_mae <= max(ceiling(length(rank_mae)*rank_flag[2,k]/100),5)) & (rank_mae >= ceiling(length(rank_mae)*rank_flag[1,k]/100)) )
		pcc_temp <- c(pcc_temp, round(cor(pred_phenotype_cal[sample_flag,])[1,2:ncol(pred_phenotype_cal)],3))	
		names(pcc_temp) <- colnames(pred_phenotype_cal)[2:ncol(pred_phenotype_cal)]
		ind_sparial_pcc_cv <- rbind(ind_sparial_pcc_cv, pcc_temp)
	}
	ind_sparial_pcc[[cv]] <- ind_sparial_pcc_cv
}
ind_sparial_pcc_result <- (ind_sparial_pcc[[1]]+ind_sparial_pcc[[2]]+ind_sparial_pcc[[3]]+ind_sparial_pcc[[4]]+ind_sparial_pcc[[5]])/5
ind_sparial_pcc_result
pdf("./figure_plot/LMP_Figure3C_spartial_PCC_bar.pdf",height=16, width=12)
par(mfcol=c(4,1))
barplot(ind_sparial_pcc_result[1,], col=col_break)
barplot(ind_sparial_pcc_result[2,], col=col_break)
barplot(ind_sparial_pcc_result[3,], col=col_break)
barplot(ind_sparial_pcc_result[4,], col=col_break)
dev.off()
...
########################----Figure 5----########################
########################----Figure 6----########################
# Figure 6, Dissecting the genetic basis of complex traits.
#--------------------------------------------------------------
# setwd('/data3/mwl/DeepAnnotation_review/gigadb')
...
options(scipen=999)
#--------------------------------------------------------------
# Figure 6A,Training and validation performance of the Genotype model across each epoch, based on the pre-trained result from 5-fold cross-validation. 
...
#--------------------------------------------------------------
# Figure 6B,Distribution of QTL detection rates across 10 simulated traits for different models.
QTL_ratio_plot <- QTL_ratio[,c(5,3,4,2,1,7,6)]
pdf("./figure_plot/LMP_Figure6B_QTL_ratio.pdf",height=16, width=12)
vivo_list <- list()
for(i in 1:ncol(QTL_ratio_plot)){
	vivo_list[[i]] <- as.numeric(QTL_ratio_plot[,i])
}
names(vivo_list) <- c("rrBLUP", "BLUP", "BayesR", "Genotype", "Network","metaG","metaN")
value_result_mean <- apply(QTL_ratio_plot,2,mean)
library(vioplot)
library(RColorBrewer)
col_break = brewer.pal(12, "Set3")
...
axis(side = 1, labels = names(vivo_list)
	...
	, at = 1:length(vivo_list))
dev.off()
heri_simu_result_plot <- heri_simu_result[,c(5,3,4,2,1,7,6)]
# ----------------------------------------------- 
# Figure 6C, Distribution of estimated heritabilities for significant SNPs (P-value < 0.05) identified by different models across 10 simulated traits.
pdf("./figure_plot/LMP_Figure6C_heri_simu.pdf",height=16, width=12)
vivo_list <- list()
for(i in 1:ncol(heri_simu_result_plot)){
	vivo_list[[i]] <- as.numeric(heri_simu_result_plot[,i])
}
names(vivo_list) <- c("rrBLUP", "BLUP", "BayesR", "Genotype", "Network","metaG","metaN")
value_result_mean <- apply(heri_simu_result_plot,2,mean)
library(vioplot)
library(RColorBrewer)
col_break = brewer.pal(12, "Set3")
col_break = col_break[c(9, 5, 6, 11, 12, 1, 2)]
...
dev.off()
# ----------------------------------------------- 
# Figure 6D, Detection rates of GWAS significant SNPs (adjusted P-value < 0.05) for the LMP trait across different models.
GWAS_ratio_plot <- GWAS_ratio[c(5,3,4,2,1,7,6)]
names(GWAS_ratio_plot) <- c("rrBLUP", "BLUP", "BayesR", "Genotype", "Network","metaG","metaN")
library(RColorBrewer)
col_break = brewer.pal(12, "Set3")
col_break = col_break[c(9, 5, 6, 11, 12, 1, 2)]
pdf("./figure_plot/LMP_Figure6D_GWAS_ratio.pdf",height=16, width=12)
barplot(GWAS_ratio_plot, col = col_break)
dev.off()
# ----------------------------------------------- 
# Figure 6E, Estimated heritabilities of significant SNPs (P-value < 0.05) identified by different models for the LMP trait.
...
pdf("./figure_plot/LMP_Figure6E_heri_real.pdf",height=16, width=12)
barplot(heri_real_result_plot, col = col_break)
dev.off()
# ----------------------------------------------- 
# Figure 6F, Manhattan plot of GWAS result for the LMP trait. Red dots highlight potential causal SNPs identified through biological interpretability of DeepAnnotation.
library(qqman)
pdf('./figure_plot/LMP_Figure6F_LMP_manhattan.pdf'
  , width = 13, height = 7)
manhattan(LMP_GWAS, chr='CHR',bp='BP',p="P",snp="SNP", col=c("#80B1D3", "#CCEBC5"), highlight = LMP_GWAS[which(as.numeric(LMP_GWAS[,"DeepAnnotation"]) < 0.01),"SNP"], suggestiveline=F, genomewideline=F)
dev.off()
t.test(-log10(LMP_GWAS[which(as.numeric(LMP_GWAS[,"DeepAnnotation"]) < 0.01),"P"]), -log10(LMP_GWAS[which(as.numeric(LMP_GWAS[,"DeepAnnotation"]) >= 0.01),"P"]), alternative = 'greater')
########################----Figure 6----########################
# ******************** Main Figures******************************** #
